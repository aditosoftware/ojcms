...
public class MapField<KEY, VALUE> extends AbstractField<MapBean<KEY, VALUE>>
{
   ...
   * @return a (modifiable) bean, that represents the original map
   */
  public MapBean<KEY, VALUE> createBeanFromMap(Map<KEY, VALUE> pMap, Class<VALUE> pValueType, @Nullable Predicate<FieldValueTuple<VALUE>> pExclude)
  {
    ...
    if (pExclude != null)
      ...
      bean.removeFieldIf(pField -> pExclude.test((FieldValueTuple<VALUE>) pField.newUntypedTuple(bean.getValueConverted(pField, pValueType))));
    return bean;
  }
